...
class LazyMinter {
   ...
   * @returns {NFTVoucher}
   */
  async createVoucher(tokenId, uri, stableCoinAddress, minStableCoinPrice, tokenAddress, minTokenPrice, isTestnet = false, isMainnet = false) {
    ...
    if (isMainnet)
      domain = await this_signingDomainMainnet();
    const types = {
      NFTVoucher: [
 ...
 */
  async _signingDomainTestnet() {
    ...
    this._domain = {
      name: SIGNING_DOMAIN_NAME,
      version: SIGNING_DOMAIN_VERSION,
      verifyingContract: "0x595D92eF4Bfe4df58a7c4a16717ea3E96d46Bf6a",
      chainId,
    }
    return this._domain
  }
...
*/
  async _signingDomainMainnet() {
    ...
    this._domain = {
      name: SIGNING_DOMAIN_NAME,
      version: SIGNING_DOMAIN_VERSION,
      verifyingContract: "0xeB5f69e409b06dD4e4D39f164f4011926B723a01",
      chainId,
    }
    return this._domain
  }
